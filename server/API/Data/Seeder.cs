...
public class AppDbSeeder
{
    ...
    private static async Task SeedTestUserAsync(AppDbContext context, UserManager<IdentityUser> userManager, IOptions<RoleSettings> roleSettings)
    {
        ...
        if (result.Succeeded)
        {
            ...
            var signalRTestUser = new IdentityUser
            {
                UserName = signalREmail,
                Email = signalREmail,
                EmailConfirmed = true
            };
            ...
            if (signalRResult.Succeeded)
            {
                ...
                var signalRTestAppUser = new User
                {
                    ...
                    Gender = 0, // SignalR TestUser is female
                    BirthDate = new DateTime(1992, 7, 25),
                    Bio = "I'm just here to match with Test User",
                    Verified = true,
                    CreatedAt = DateTime.UtcNow,
                    LastActivityDate = DateTime.UtcNow,
                    Interests = ["Coding", "Playing Video Games"],
                    PreferredMinAge = 20,
                    PreferredMaxAge = 100,
                    PreferredLocationRange = 50,
                    PreferredGender = 2, // SignalR TestUser prefers both
                    UserLocation = new UserLocation
                        { Latitude = 47.4979M, Longitude = 19.0402M },
                    Photos =
                        new List<Photo>()
                        {
                            new()
                            {
                                Url = "https://i.pinimg.com/736x/9c/6c/4d/9c6c4d0e9bb7fd906c85304497f90fd1.jpg"
                            },
                            new()
                            {
                                Url = "https://i.pinimg.com/736x/a9/e2/b7/a9e2b7b1071cf21ab1cb64931f28e580.jpg"
                            },
                            new()
                            {
                                Url = "https://i.pinimg.com/originals/f5/ba/22/f5ba22d597106e62396b1f36772a07f6.jpg"
                            }
                        }
                };

                signalRTestAppUser.AspNetUserId = signalRTestUser.Id;
                context.Users.Add(signalRTestAppUser);
                await context.SaveChangesAsync();
            }
        }
    }
