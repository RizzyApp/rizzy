...
const ProfilePage = () => {
  ...
  const { showPromiseToast, showErrorToast, showSuccessToast } =
    useCustomToast();
  ...
  const handleSaveChanges = async () => {
    setIsUploading(true);
    try {
      const { metadata, formData } = createPictureChangesFormData(
        initialPhotos,
        changedPhotoUrls
      );

      // Prepare the profile update promise
      const profileUpdatePromise = fetchWithAuth(
        API_ENDPOINTS.USER.PUT_PROFILE,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (metadata.some((item) => item.action !== "KEEP")) {
        photoUpdatePromise = fetchWithAuth(
          API_ENDPOINTS.IMAGE.POST_IMAGE_CHANGES,
          {
            method: "POST",
            body: formData,
          }
